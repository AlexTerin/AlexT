...
class BodyMassIndexApp(Frame):
    ...
    def gui(self):
        ...
        Label(self.mainFrame, text='Stones:').grid(column=0, row=2, sticky='E')
        ...
        Label(self.mainFrame, text='Feet:').grid(column=2, row=2, sticky='E')
        ...
        self.calculateButton = Button(self.mainFrame, text='Calculate BMI', width=10, command=self.check_data)
    ...
    #If BMI was calculated, the result will be added to the file, otherwise, only data entries will be saved
    ...
    def check_nav_button(self, value):
        try:
            ...
            mb.showwarning('No entries', 'No entries, please update records')
    ...
    #BMI calculation logic, checks the user input, gets the data, checks the data, calculates BMI
    def check_data(self):
        if self.weightInput.get() is None or self.heightInput.get() is None or self.weightInput.get() == '' or self.heightInput.get() == '':
            self.get_data_imperial()
        else:
            self.get_data()
    ...
    def get_data_imperial(self):
        try:
            ...
            self.weight = (stones * 14 + pounds) * 0.453592
            self.height = (feet * 12 + inches) / 39.3701
            self.calculate()
    ...
    def calculate(self):
        try:
            ...
            if 2 < self.weight < 650 and 0.5 < self.height < 2.7:
